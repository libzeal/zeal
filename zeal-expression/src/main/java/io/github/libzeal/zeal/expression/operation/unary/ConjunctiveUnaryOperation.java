...
/**
 * A compound operation where all sub-operation must be pass for this compound operation to pass.
 *
 * @param <T>
 ...
 * @since 0.2.0
 */
public class ConjunctiveUnaryOperation<T> implements CompoundUnaryOperation<T> {
     ...
     */
    public ConjunctiveUnaryOperation(String name, List<UnaryOperation<T>> operations) {
        this.name = requireNonNull(name);
        this.operations = requireNonNull(operations);
    }

    /**
     * Creates a new conjunctive operation with an empty default sub-operation list.
     *
     * @param name
     *     The name of the operation.
     *
     ...
     */
    public ConjunctiveUnaryOperation(String name) {
    ...
    public Evaluation evaluate(final T subject) {
        ...
        final SkippableCompoundUnaryOperationEvaluator<T> evaluator = new SkippableCompoundUnaryOperationEvaluator<>(
            name,
            (passed, failed, skipped, total) -> failed > 0,
            this::computeResult,
    ...
    private Result computeResult(final EvaluationContext<T> context) {
        ...
        else if (context.passed() == context.total()) {
            return PASSED;
        }
        else {
            return SKIPPED;
